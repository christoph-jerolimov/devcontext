...
**Sync now** button. A sync can run for hours, and for exactly that reason it
is never silent: the sidebar shows a progress bar with the percentage, the
estimated time left and the item being fetched right now, fed by the same
counters as the CLI's own progress line — and a page opened in the middle of
a run learns about it immediately from `/api/status`, not from the next
event. The serving terminal logs a line at every 10% too. The events fire on _any_ write to the database, so a
...
| Tickets         | GitHub issues and Jira work items in one list, each with the people who worked on it. Quick filters for source, repository/project, type and state — the type and container dropdowns are built from the data, so a project that invents a type gets an entry without a redeploy                                                                         |
...
| Pull requests   | Same, plus reviews with their inline comments and the changed files, and a people column carrying the reviewers as well as the author. Also every state, since a merged pull request is the normal end of one                                                                                                                                            |
...
| History         | Five charts over the last 30 days to a year: how many tickets, GitHub issues and pull requests were open per day, how many pull requests were finished per day split by merged or thrown away, and how many workflow runs ran per day split by conclusion. Filterable by repository, and the finished chart by person or team. See [History](history.md) |
| Insights        | Cycle time, review latency, WIP, reviewers, flaky steps, time in each status and stale work, with adjustable windows                                                                                                                                                                                                                                     |
