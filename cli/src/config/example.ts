...
sync:
  ...
  minDelayMs: 250
  # How many API calls may run at the same time. The spacing above still
  # applies between any two request starts, so this hides latency rather than
  # multiplying throughput; 1 restores the strictly serial sync.
  concurrency: 4
  maxRetries: 5
  retryBaseMs: 1000
  ...
  rateLimitReserve: 50
  # Fail with a clear error instead of sitting out a rate limit window that is
  # longer than this. 15 minutes by default.
